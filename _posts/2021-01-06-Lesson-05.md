...
When it comes to actually building websites, a lot of data, e.g., images, are needed. All of you should be aware how much storage space images can take on your hard-drive. When it comes to websites, this problem gets even worse.
...
Certainly, the provider’s speed, your internet connection, and many other factors have something to say here as well, but in the end, we want to make a website appealing for everyone, so we try to make it as small as possible.
...
Especially in languages like German or Swedish, some of these special letters like “ü,” “ö,” or “å.” Computers do not like those, meaning you should avoid them. But what is even more annoying, computers do not like _spaces_ as well! Your operating system has a well-thought-through code to prevent this problem, but unfortunately, the web and browsers often have not. Therefore you should replace all your spacings with underscores `_` or minus `-` symbols.  

Another part is that your some of your code’s languages look at the different spellings with capitals and other languages do not. Therefore I would propose to avoid using _capitals_ as well.  

Also, try to avoid _dots_. Nowadays, it should not be a problem anymore, but I always try to stay safe and do not use them in names if some old browsers might have some troubles. But now that should finally be it!  
...
That is the way you navigate to files! Always keep in mind in which file you are right now and how often you need to step one folder back or which folder further in. That can be as complex as you want: `<a href="../../works/year_one/portfolio.html">That is my portfolio</a>`.
...
[^1]: Average from 2018. Read more about it [here](https://www.seoptimer.com/blog/webpage-size/).